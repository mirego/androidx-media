  ...
  private static boolean areSizeAndRateSupported(
    ...
    if (frameRate == Format.NO_VALUE || frameRate < 1 || shouldIgnoreCodecFpsLimitForResolution) {
      ...
      if (!capabilities.areSizeAndRateSupported(width, height, floorFrameRate)) {
        ...
        Log.v(Log.LOG_LEVEL_VERBOSE1, TAG, "isSizeSupported: %s  supported frame rates: %s",
            capabilities.isSizeSupported(width, height), capabilities.getSupportedFrameRates());
        // MIREGO END
        return false;
      }
      if (SDK_INT < 24) {
